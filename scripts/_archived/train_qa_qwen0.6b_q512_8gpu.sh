#!/bin/bash
# Training script for Qwen3-0.6B with Q=512 on 8 GPUs

set -e

echo "========================================================================"
echo "Deep Compressor QA Training - Qwen3-0.6B, Q=512, 8 GPUs"
echo "========================================================================"
echo "Start time: $(date)"
echo ""

OUTPUT_DIR="outputs/qa_qwen0.6b_q512_8gpu"
...
echo "Configuration:"
echo "  Model:                Qwen3-0.6B (hidden=1024, layers=28)"
echo "  Q value:              512"
echo "  GPUs:                 8"
echo "  Batch size (per GPU): 16"
echo "  Gradient accum:       2"
echo "  Effective batch:      $((8 * 16 * 2))"
...
accelerate launch \
    ...
    --config configs/qa_qwen0.6b_q512_8gpu.yaml \
    --data_path "$DATA_PATH" \
...
if [ $EXIT_CODE -eq 0 ]; then
    echo "Qwen3-0.6B Q=512 training completed successfully!"
else
    echo "Qwen3-0.6B Q=512 training failed with exit code $EXIT_CODE"
fi
echo "End time: $(date)"
echo "========================================================================"
