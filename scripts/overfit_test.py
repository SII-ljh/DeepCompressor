#!/usr/bin/env python3
"""NTP overfit stress test: load 5% real data and train until PPL drops.

Goal: verify that the compression pipeline can actually reduce NTP loss /
perplexity when trained hard on a small data subset.  This isolates Stage 1
(compress doc → prefix → predict continuation) from any QA / distillation
complexity.

The script:
  1. Loads the 5% ablation NTP subset (data/ablation/ntp_ablation.jsonl)
  2. Splits 90/10 into train/val
  3. Trains NTP for many steps with aggressive LR, no regularisation
  4. Logs train loss + val PPL periodically
...
logger = logging.getLogger("overfit_ntp")


# ── Config ────────────────────────────────────────────────────────────

def build_config(args) -> DeepCompressorConfig:
    """Build config tuned for aggressive NTP overfitting."""
...
def evaluate_ntp_simple(model, eval_loader, device, use_bf16=False):
    ...
    ppl = math.exp(min(avg_loss, 20))  # cap to avoid inf
    model.train()
...
def train(model, train_loader, val_loader, config, device, args, use_bf16=False):
    ...
    # Cosine or linear schedule with warmup
        ...
        for batch in train_loader:
            ...
            if micro_steps % tcfg.gradient_accumulation_steps == 0:
                ...
                if step % tcfg.log_every == 0:
                    ...
                    logger.info(
                        f"[NTP] step {step}/{tcfg.max_steps}  "
...
def main():
    parser = argparse.ArgumentParser(
        description="NTP overfit stress test: train hard on 5%% data, track PPL")
    ...
    parser.add_argument("--max_doc_tokens", type=int, default=512,
                        help="Max document tokens (default: 512)")
    ...
    parser.add_argument("--log_every", type=int, default=50,
                        help="Log train loss every N steps")
    parser.add_argument("--eval_every", type=int, default=500,
                        help="Evaluate val PPL every N steps")
    ...
    logger.info("Loading Qwen3-0.6B + DeepCompressor...")
    ...
    if best_ppl < 50:
        print("\n  RESULT: PPL < 50 — compression is working, prefix carries info")
    elif best_ppl < 200:
        print("\n  RESULT: PPL 50-200 — partial compression, may need more steps or tuning")
    else:
        print("\n  RESULT: PPL > 200 — compression not effective, investigate architecture")
    print("=" * 60)
