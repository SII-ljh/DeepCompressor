#!/usr/bin/env python3
"""Step 2: Memorize 32 samples — H200 single-batch overfit (~10 min).

Minimal model: identity projections, Stage A only (1 cross + 1 self),
Stage B/C disabled. ~29.5M trainable params.

Uses first 32 samples as one full batch — no val split, pure memorization.
Goal: train loss drops hard, PPL approaches 1.

Usage:
  python scripts/overfitting/step2_memorize_tiny.py
  python scripts/overfitting/step2_memorize_tiny.py --steps 3000 --lr 5e-4
  python scripts/overfitting/step2_memorize_tiny.py --data_path data/ntp_tiny.jsonl
"""
...
def build_config(args) -> DeepCompressorConfig:
    """Minimal model for tiny-set memorization."""
...
def train(model, train_loader, config, device, args, use_bf16=False):
    """Single-batch NTP training loop — no val split, pure memorization."""
    ...
    # Initial eval on training batch
    ...
    while step < tcfg.max_steps:
        ...
        for batch in train_loader:
            ...
            if running_count % tcfg.gradient_accumulation_steps == 0:
...
def main():
    parser = argparse.ArgumentParser(
        description="Step 2: Memorize 32 samples in one batch (H200, ~10 min)")
    ...
    parser.add_argument("--batch_size", type=int, default=32,
                        help="Batch size (default: 32, = num_samples)")
    ...
    parser.add_argument("--eval_every", type=int, default=200,
                        help="Evaluate val PPL every N steps (default: 200)")
    ...
    logger.info(f"Queries: {args.num_queries}  Samples: {args.num_samples}")
    ...
    # Dataset: first N samples, no val split — pure memorization
    full_ds = NTPDataset(
    ...
    logger.info(f"Dataset: {len(full_ds)} total, using {n_use} samples (1 batch, no val)")
    ...
    # Train (no val — pure memorization)
    metrics = train(model, train_loader, config, device, args, use_bf16=use_bf16)

    # Summary
    print("\n" + "=" * 60)
    print("  STEP 2: MEMORIZE 32 SAMPLES (SINGLE BATCH)")
    print("=" * 60)
    ...
    print(f"  Data:         {args.data_path} ({n_use} samples, 1 batch)")
