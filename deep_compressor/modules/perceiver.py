...
class PerceiverCrossAttention(nn.Module):
    """Cross-attention with optional anchor_scores bias."""

    def __init__(self, dim: int, num_heads: int, head_dim: int, dropout: float = 0.1,
                 use_anchor_bias: bool = False, anchor_score_scale_init: float = 1.0):
        super().__init__()
        ...
        self.use_anchor_bias = use_anchor_bias
        if use_anchor_bias:
            self.anchor_scale = nn.Parameter(torch.tensor(anchor_score_scale_init))

    def forward(self, q: torch.Tensor, kv: torch.Tensor,
                kv_mask: Optional[torch.Tensor] = None,
                anchor_scores: Optional[torch.Tensor] = None) -> torch.Tensor:
        """
        Args:
            q: (batch, q_len, dim) — queries
            kv: (batch, kv_len, dim) — key-value source
            kv_mask: (batch, kv_len) — True for valid positions
            anchor_scores: (batch, kv_len) — per-token entity scores
        Returns:
            (batch, q_len, dim)
        """
        residual = q
        ...
        if self.use_anchor_bias and anchor_scores is not None:
            # anchor_scores: (B, S) → (B, 1, 1, S)
            attn = attn + self.anchor_scale * anchor_scores.unsqueeze(1).unsqueeze(1)
...
class PerceiverBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, kv: Optional[torch.Tensor] = None,
        ...
        if self.has_cross_attn:
            ...
            x = self.cross_attn(x, kv, kv_mask=kv_mask, anchor_scores=anchor_scores)
        x = self.self_attn(x)
        x = self.ff(x)
        return x
...
class GuidedPerceiver(nn.Module):
    ...
    Stage B: FinBERT ON → anchor cross-attn; FinBERT OFF → self-attention
    ...
    def forward(self, queries: torch.Tensor, byte_array: torch.Tensor,
        ...
        Args:
            ...
            anchor_scores: (batch, doc_len) — per-token entity scores (FinBERT only)
            anchor_embs: (batch, top_k, dim) — anchor embeddings (FinBERT only)
        Returns:
